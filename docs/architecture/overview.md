...
The renderer APIs remain runtime-only. The browser maps resolved Design 1.0 presentation overrides and effective Material 1.0 finishes to these contracts. Persistent paths never enter the runtime material model. Environment assets, material-management UI, lighting design, and post-processing remain future work.
...
Implements the independent pure PlanAxis Material Format 1.0 boundary with no runtime dependencies. `parseMaterialDescriptor(text, descriptorPath)` parses decoded JSON text; `validateMaterialDescriptor(value, descriptorPath)` checks untrusted parsed values and the exact project-relative `.json` identity under `assets/materials/`. Success returns a nominal, deeply immutable `ValidatedMaterialDescriptor` with external `path` and a separate `document` preserving omitted optional fields.

Validation enforces recursively closed objects, finite scalar ranges, RGB tuples, map/mapping coupling, positive physical dimensions, supported texture-reference paths, and discriminated alpha variants. Expected failures return `MaterialValidationResult` with a JSON/format stage, stable `MATERIAL_*` code, field location, and message. Descriptor-path errors have a distinct code and `descriptorPath` location.

`getEffectiveMaterial(descriptor)` returns immutable effective scalar/alpha values using the [normative Material 1.0 defaults](../specifications/planaxis-material/1.0.md#20-defaults-and-normalized-interpretation), without changing or rewriting the durable document. Map roles retain explicit color/channel semantics and common mapping dimensions. All Material Format numeric fields use ordinary finite JavaScript numbers, independently of authoritative architectural geometry.
