...
The filesystem-backed project layer is adopted by ADR-004 and PlanAxis Project Format 1.0. Its server-side loading, filesystem boundary, startup project selection, controlled HTTP APIs, and browser acquisition of the active Apartment SVG are implemented as Phase 0. Phase 2 implements shared Design 1.0 validation/resolution, server design discovery/persistence APIs, and the browser scenario workflow. Section 16 distinguishes implemented behavior from accepted-but-not-yet-implemented architecture.
...
The project format is independently versioned from Apartment SVG, PlanAxis Design Format, PlanAxis Material Format, and future model-asset formats.
...
`RuntimePbrTextures` gives all maps one physical repeat width/height. The adapter computes `u = physicalUDistanceCm / widthCm` and `v = physicalVDistanceCm / heightCm`, permitting negative UVs and repeats beyond one. Base-color maps use sRGB; roughness (green channel), metalness (blue channel), ambient occlusion (red channel), and tangent-space normal maps use non-color data. Normal +Y follows mapping +V. All maps use UV channel zero, repeat wrapping, and identity texture transforms; repeat density comes entirely from physical dimensions. The resolver must provide consistently oriented, already loaded pixels (+U right / +V up), using the source texture's upload orientation. Routine tests resolve deterministic in-memory `DataTexture` fixtures.

Runtime AO map references and effective `ambientOcclusionStrength` must be supplied together; strength is validated as finite in [0,1]. The adapter sets the standard Three.js `aoMap` and `aoMapIntensity`, realizing `effectiveAO = 1 - strength * (1 - redSample)` for ambient/indirect illumination. AO uses the same physical mapping and UV channel zero without additional geometry attributes. Packed ORM roles borrow one decoded source image through separately configured texture clones.

Masked finishes use an explicit node mask with `sampledAlpha × opacity >= cutoff`, including equality at 0 and 1; this avoids Three.js's default node alpha-test equality discard and also applies to shadow masks. Opaque and blended finishes retain standard PBR adaptation.
...
After backend initialization, the renderer generates one 256-pixel-face PMREM using the built-in deterministic Three.js `RoomEnvironment` and the WebGPU-compatible `PMREMGenerator`. No project assets or network images are needed. The resulting texture is assigned only to `scene.environment`; the background remains neutral `0xe8ecec`. Environment intensity and yaw use scene environment properties and affect PBR lighting/reflections without moving geometry or cameras. Hemisphere illumination is removed. The existing directional key/shadow light retains its apartment-relative positioning, 2048-pixel shadow map, and bounds-scaled framing/bias. Environment generation resources are released after generation; the output target/texture lasts for the renderer instance and is released on disposal. Initialization failure and disposal during pending backend initialization retain explicit cleanup paths.

React owns the transient presentation selections and passes full updates to the renderer. Toolbar controls use intensity 0–4 / step 0.1, yaw 0–360° / step 1°, and exposure −4–4 EV / step 0.1. They are disabled until readiness and hidden with the toolbar in Focus view. Settings survive orbit, embedded-camera selection, Walk, focal-length changes, resize, and model replacement in the same renderer instance. Updates neither rebuild architectural models nor start a persistent render loop.
...
Successful validation also constructs `ArchitecturalModel3D` through its public builder. Valid documents start in 2D and expose a 3D switch without reparsing. The 3D view offers orbit inspection, embedded cameras, and Walk, with horizontal FOV adapted on resize. Walk is disabled with an explanatory message when no embedded camera exists. The browser owns the camera/view selector, independent focal-length and render-aspect controls, transient presentation controls (section 5.9), and concise navigation help. Focus view changes the layout without remounting the renderer or resetting its pose and projection selections. Unmount and replacement release renderer resources; failures remain explicit application states. Invalid documents retain only 2D preview and diagnostics.

Vite development proxies only the project metadata, active-architecture, architecture-resource, design discovery, design read/write, material descriptor, and material-texture API paths to `http://127.0.0.1:3000` without rewriting them. Application requests remain same-origin; Fastify needs no permissive CORS. The browser has no local SVG picker/drop-loading fallback or runtime project switch. Restart the server and reload the browser to select another project. Reload the page to reread active SVG bytes; polling and filesystem watching are not implemented.

The browser scenario workflow is implemented in small application adapters: `design-api.ts` owns controlled HTTP transport, `load-design.ts` composes shared format validation with the existing `processDocument` SVG pipeline and `deriveArchitecturalSurfaces(...).finishTargets`, and `design-presentation.ts` explicitly maps presentation vocabulary. `use-design.ts` owns session selection, loading, resolution, and persistence state; `design-panel.tsx` presents discovery, diagnostics, and create/edit forms. Design semantics remain in `@planaxis/design`.

Discovery never auto-selects a descriptor, and failure leaves the active-architecture workspace usable. A selected design loads its exact bound SVG through `architecture-resource`; no manifest fallback substitutes for that binding. Clearing selection restores the startup active-architecture result. Selection uses no local storage and writes no project data. Abort signals and request generations prevent obsolete selection or write responses from replacing newer state.

Structurally valid descriptors remain editable even when their architecture cannot resolve. JSON/format failures, project/API resource failures, SVG diagnostics, stale targets, and application/renderer failures remain separate. Invalid SVGs retain the existing diagnostic viewer; valid architecture with stale targets uses default appearance. Persisted finish assignments remain unchanged during editing. After successful architecture resolution, the browser translates effective Material 1.0 / 1.1 semantics into runtime PBR assignments.

`load-materials.ts` fetches each distinct exact material identity through `/api/project/material`, parses it with `@planaxis/material`, and uses `getEffectiveMaterial` for defaults. It fetches each distinct referenced texture through `/api/project/material-texture`, including shared images and packed map roles. JSON-number physical dimensions are adapted to the existing runtime decimal type without changing persistent semantics. Descriptors and fetched bytes remain renderer-neutral; no Three.js objects are constructed by the application.

`prepareRuntimeTextures` in `renderer-three` verifies PNG/JPEG/WebP signatures against extensions before image decoding. It decodes each image once with vertical flipping, no premultiplied alpha, and no color-space conversion; source textures disable further flipping. This preserves image-right = +U and image-up = +V without inverting normal Y. The returned handle owns source textures and image bitmaps; scenes own their configured role-specific clones. Preparation releases partial allocations on failure or abort, including late decoder completions. Selection cleanup releases source handles after child scene cleanup. A temporary 2D/3D switch releases scene resources while retaining the selection's prepared sources for reuse.

All persistent finishes apply atomically after preparation. Descriptor API, Material JSON/format, texture API, content mismatch, decode, and renderer adaptation failures have separate safe diagnostics. Failure preserves valid architecture and resolved presentation with neutral finishes. The viewport retries neutral scene construction on persistent material rendering/adaptation failure; an unrecoverable backend or neutral-rendering failure retains the existing renderer-failure workflow. Generation and abort checks prevent obsolete results from replacing a newer selection. No finishes means no material-resource requests. Re-selecting a scenario rereads its resources; there is no global cache or file watching.

Create uses POST with required fields bound to the currently displayed architecture, then refreshes discovery and selects the created path. Save uses PUT for the same descriptor identity and preserves architecture/finishes. Name and presentation drafts become durable only after successful writes; failures leave the last loaded descriptor intact. Removing either presentation field preserves the other's optional semantics; removing both omits the object.

Resolved scenarios apply saved tone mapping and exposure through the existing renderer API. The design editor owns these edits while selected; the toolbar displays their effective values. Omitted values use current defaults, and finite exposures are not clamped to slider bounds. Renderer multiplier limitations remain renderer failures. Environment intensity/rotation remain transient and are excluded from writes. Saving presentation does not rebuild the architecture or reset cameras.
...
Design routes add recursive candidate discovery, raw descriptor reads, validated creates/updates under `designs/`, and raw reads of explicitly selected architecture alternatives. The concise [HTTP contract](../../README.md#adopted-project-based-workflow) documents endpoints and responses. Route handlers use `@planaxis/design` for structural write validation and serialize only its validated `document`; no server SVG validation, finish-target resolution, material resolution, or active-design state is introduced. Malformed and unsupported descriptors remain discoverable and readable. Expected input/filesystem failures produce controlled responses, while unexpected infrastructure failures are logged server-side and returned as generic HTTP 500 errors.

`GET /api/project/material?path=<project-relative-path>` reads one lowercase `.json` descriptor candidate below `assets/materials/`; `GET /api/project/material-texture?path=<project-relative-path>` reads one lowercase `.png`, `.jpg`, `.jpeg`, or `.webp` resource anywhere below the same directory. Both require exactly one `path` parameter and reject unknown parameters. The shared controlled resource routes return unchanged bytes with `application/octet-stream` and `X-Content-Type-Options: nosniff`, using the same safe project-error handling as design reads. Each read re-enters `ProjectFilesystem` for containment, regular-file, and symbolic-link checks. The server does not parse or validate Material JSON, decode textures, or inspect their content; malformed descriptors and undecodable texture bytes remain readable. Material discovery and write endpoints are absent. The browser uses both endpoints through the narrow Vite proxy and performs downstream material/texture resolution before supplying runtime assignments to the renderer.

The project root is not statically mounted. Resources outside these deliberate endpoints and `.planaxis/` are not exposed. The server does not serve the browser application. Vite provides the development-only same-origin API proxy described in section 8.1; the server adds no CORS integration.
...
`listFiles(directory, suffix)` recursively discovers accessible regular candidates without reading their contents, skips symbolic links and inaccessible entries, and sorts project-relative paths lexicographically. Every candidate read re-enters the boundary. `writeDesignFile(path, bytes, mode)` restricts durable writes to descriptor paths under `designs/`. It creates parents one component at a time after checking the preceding directory, and stages a complete UTF-8 file in the destination directory with exclusive/no-follow flags. The file is synced before publication: create uses a non-overwriting hard link; update rechecks the existing regular target and uses atomic rename. Failed staging leaves existing descriptors untouched, and temporary files are removed through fresh boundary checks. Directory entries are synced where the host supports it; filesystems without hard-link publication fail safely instead of falling back to a partial create. These portable checks retain the concurrent local directory-replacement limitation described above and provide no multi-process transaction or locking contract.

Startup and HTTP integration use this foundation as described in section 8.2. Runtime project switching remains unimplemented.
...
Implements the independent pure PlanAxis Material Format 1.0 / 1.1 boundary with no runtime dependencies. `parseMaterialDescriptor(text, descriptorPath)` parses decoded JSON text; `validateMaterialDescriptor(value, descriptorPath)` checks untrusted parsed values and the exact project-relative `.json` identity under `assets/materials/`. Success returns a nominal, deeply immutable `ValidatedMaterialDescriptor` with external `path` and a separate `document` preserving omitted optional fields.

Validation enforces version-specific recursively closed objects, finite scalar ranges, RGB tuples, map/mapping coupling, Material 1.1 AO map/strength coupling, positive physical dimensions, supported texture-reference paths, and discriminated alpha variants. Expected failures return `MaterialValidationResult` with a JSON/format stage, stable `MATERIAL_*` code, field location, and message. Descriptor-path errors have a distinct code and `descriptorPath` location.
