...
The Phase 1 renderer APIs remain runtime-only. The Phase 2 browser maps resolved Design 1.0 tone-mapping/exposure overrides to this API; persisted finish references never become runtime material assignments in this phase. Material Format 1.0 now defines persistent material semantics, but browser material loading and conversion to runtime assignments remain Phase 3 implementation work. Environment assets, lighting design, and post-processing remain future work; no material/environment resource API or material-selection UI is introduced by Phase 1.
...
Other future persistent project/design facts may include resolved material assets, imported objects, lighting design, environment assets, and saved generated outputs. Those contracts must be defined by their own accepted formats rather than guessed into the current project manifest or Design Format 1.0.
...
Vite development proxies only the project metadata, active-architecture, architecture-resource, design discovery, and design read/write API paths to `http://127.0.0.1:3000` without rewriting them. Application requests remain same-origin; Fastify needs no permissive CORS. The browser has no local SVG picker/drop-loading fallback or runtime project switch. Restart the server and reload the browser to select another project. Reload the page to reread active SVG bytes; polling and filesystem watching are not implemented.
...
This package has no dependency on `design`, `model-3d`, applications, filesystem APIs, browsers, React, or Three.js. Resource existence, project-root containment, symbolic-link checks, image decoding, and runtime PBR adaptation remain separate later stages. Its introduction does not alter Design 1.0 material-reference conformance or the current browser/rendering workflow.
