...
ADR-004 and PlanAxis Project Format 1.0 adopt a filesystem-backed project as the future top-level application container. The server-side loading foundation in `apps/server/src/project/` validates manifests and required structure and provides a read-only filesystem boundary for one canonical root. Apartment SVG contents remain independently validated downstream. The server requires one `--project <path>` at startup, loads it before listening on `127.0.0.1:3000`, and exposes controlled project metadata and active-architecture HTTP APIs. Browser migration is not implemented yet; the browser still uses its local file picker and drag-and-drop workflow.
