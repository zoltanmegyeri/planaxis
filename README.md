...
The browser uses these project and design APIs through the development-only Vite proxy described above. Fastify does not serve the React build; the two-process development flow is the supported browser startup workflow. Server design persistence and browser scenario selection/editing are implemented. Persistent material assets and rendering remain Phase 3 work; richer redesign is deferred.
