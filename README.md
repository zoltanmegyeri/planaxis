...
The versioned [PlanAxis Material Format 1.0 specification](docs/specifications/planaxis-material/1.0.md) now defines the accepted persistent contract for project-local reusable PBR materials under `assets/materials/`, including scalar metallic/roughness properties, PNG/JPEG/WebP texture maps, common physical texture dimensions, and alpha behavior. The shared `@planaxis/material` package implements pure JSON and descriptor-path validation, trusted immutable descriptors, and effective scalar/alpha defaults. Read-only server material-resource APIs now provide controlled raw descriptor and texture access. Browser material resolution and persistent PBR rendering remain Phase 3 work.
...
- `GET /api/project/material-texture?path=assets/materials/shared/base-color.webp`: unchanged bytes of one explicitly requested lowercase `.png`, `.jpg`, `.jpeg`, or `.webp` file below `assets/materials/`, with octet-stream/nosniff headers. Textures may be anywhere below that directory, independently of descriptor location.
...
The browser uses the project and design APIs through the development-only Vite proxy described above. Material endpoints are server-only for now; browser fetching and Vite proxy support for them remain pending. Fastify does not serve the React build; the two-process development flow is the supported browser startup workflow. Server design persistence and browser scenario selection/editing are implemented. Browser material loading and persistent rendering remain Phase 3 work; richer redesign is deferred.
...
PlanAxis Design Format 1.0 is the accepted normative persistence contract for Phase 2 design scenarios. `@planaxis/design` provides `parseDesignDescriptor(text, descriptorPath)` and `validateDesignDescriptor(value, descriptorPath)`, returning an immutable, format-conformant descriptor with external `path` identity and a separate `document` containing only serialized fields. JSON syntax and format failures have structured codes and field locations. `resolveDesignArchitecture(design, { path, finishTargets })` checks exact binding and reports all unresolved targets against caller-supplied targets derived from a fully validated architecture. The package performs no filesystem access, SVG loading, material interpretation, or rendering. Server design APIs use this structural boundary for persistence. The browser uses it for scenario selection, bound-architecture loading, finish-target resolution, and explicit creation/editing, completing Phase 2. PlanAxis Material Format 1.0 validation is implemented in `@planaxis/material`; controlled raw material-resource reads are implemented on the server, while browser material resolution and rendering remain Phase 3 work.

`@planaxis/material` exposes `parseMaterialDescriptor(text, descriptorPath)` and `validateMaterialDescriptor(value, descriptorPath)`. They return structured JSON/format failures or an immutable descriptor with exact project-relative `path` identity and a separate durable `document`. `getEffectiveMaterial(descriptor)` exposes normative defaults without modifying the document. The package has no runtime dependencies and performs no filesystem access, image decoding, or renderer adaptation. See the [Material Format specification](docs/specifications/planaxis-material/1.0.md) for the normative contract.
